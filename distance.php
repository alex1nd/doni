...
function handleLeadStatusChange($leadId) {
    ...
    $source = $leadData['result']['SOURCE_ID'];
    $meetingDateTime = $leadData['result']['UF_CRM_1579787937'];
    $eventTitle = "Дистанционно в {$source}, {$meetingDateTime}";
    ...
    $response = createCalendarEvent($responsibleId, $sectionId, $eventTitle, $meetingDateTime, $leadId);
...
// Получение секции календаря пользователя
...
function createCalendarEvent($userId, $sectionId, $title, $dateTime, $leadId) {
    ...
    $queryData = [
        ...
        'ownerId' => $userId,
        'name' => $title,
        'description' => 'Событие создано автоматически при изменении статуса лида.',
        'from' => $dateFrom,
        'to' => $dateTo,
        'skipTime' => 'Y',
        'section' => $sectionId,
...
// Пример использования: вызов функции handleLeadStatusChange с ID лида
